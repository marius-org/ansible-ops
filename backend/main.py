...
app = FastAPI(title="Ansible Ops API")
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://ansible:ansiblepass@postgres-service:5432/ansibledb")
pool = None
...
async def post_score(data: ScoreIn):
    if not pool: raise HTTPException(503, "DB unavailable")
    if data.score < 0 or data.score > 99999: raise HTTPException(400, "Invalid score")
    name = data.player_name.strip()[:50] or "ENGINEER"
    async with pool.acquire() as conn:
